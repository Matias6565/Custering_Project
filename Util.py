...
def get_Fog_LambdasNecessary(graph,i):
  total = 0.0
  tot_traffic = i*ecpri
  print("tot_traffic {}".format(tot_traffic))
  total = math.ceil((tot_traffic - graph["bridge"]["cloud"]["capacity"])/10000)
  #print(total)
  return total+1
...
def distancia(value):
  df = pd.read_csv('antenas2.csv', delimiter = ';')  
...
def total_D(i, graph):
    arrival_rate = sim.arrival_rate
    cloud_service_rate = 0.00001
    # Verifica se "graph" é um dicionário antes de tentar acessar seus elementos
    if isinstance(graph, dict) and "bridge" in graph and "cloud" in graph["bridge"]:
        if graph["bridge"]["cloud"]["capacity"] > 0:
        ...
        # Se "graph" não for um dicionário válido, retorne um valor padrão ou trate conforme necessário
        return 0
